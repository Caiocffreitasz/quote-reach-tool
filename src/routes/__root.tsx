  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-chassis px-4">
      <div className="max-w-md text-center">
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-tool border-4 border-grip">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-8 w-8 text-grip"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <h1 className="text-2xl font-black uppercase tracking-tight text-grip">Algo deu errado</h1>
        <p className="mt-2 text-sm text-chassis-dark font-semibold">
          Ocorreu um erro inesperado. Tente novamente.
        </p>
        {import.meta.env.DEV && error.message && (
          <pre className="mt-4 max-h-40 overflow-auto rounded-lg border-4 border-grip bg-card p-3 text-left font-mono text-xs text-grip">
            {error.message}
          </pre>
        )}
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          <Link
            to="/"
            className="inline-flex items-center justify-center rounded-lg border-4 border-grip bg-card px-6 py-3 text-sm font-black uppercase tracking-wider text-grip shadow-brutal-sm hover:translate-y-[2px] hover:shadow-none transition-all"
          >
            Voltar ao início
          </Link>
        </div>
